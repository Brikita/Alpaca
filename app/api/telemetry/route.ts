import { isAlpacaSnapshot } from '../../../lib/telemetry-contract';
import {
...
export async function GET(): Promise<Response> {
  const snapshot = await latestTelemetrySnapshot();
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!expectedToken) {
    return Response.json({ error: 'Telemetry ingestion is not configured.' }, { status: 503 });
  }
  ...
  if (!isAlpacaSnapshot(payload)) {
    return Response.json({ error: 'Invalid telemetry snapshot.' }, { status: 422 });
  }
